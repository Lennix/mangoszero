    ...
    for(int i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i)
    {
        ...
        if (traderCanTrade && playerCanTrade )
        {
            ...
            if (myItems[i])
            {
                ...
                if (_player->GetSession()->GetSecurity() >= SEC_GAMEMASTER && sWorld.getConfig(CONFIG_BOOL_GM_LOG_TRADE))
                {
            ...
            if (hisItems[i])
            {
                ...
                if (trader->GetSession()->GetSecurity() >= SEC_GAMEMASTER && sWorld.getConfig(CONFIG_BOOL_GM_LOG_TRADE))
                {
...
void WorldSession::HandleAcceptTradeOpcode(WorldPacket& recvPacket)
{
    ...
    if (his_trade->IsAccepted())
    {
        ...
        {
            if (_player->GetSession()->GetSecurity() >= SEC_GAMEMASTER && my_trade->GetMoney() > 0)
            {
            ...
            if (trader->GetSession()->GetSecurity() >= SEC_GAMEMASTER && his_trade->GetMoney() > 0)
            {
