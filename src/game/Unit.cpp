...
bool Unit::AddSpellAuraHolder(SpellAuraHolder *holder)
{
    ...
    if ((!(aurSpellInfo->Attributes & (SPELL_ATTR_PASSIVE | SPELL_ATTR_UNK7)) && !holder->IsPersistent()) || holder->IsAreaAura())
    {
...
bool Unit::IsMorePowerfulSpellActive(uint32 spellId, Unit* caster)
{
    ...
    //{
        ...
        for (int eff = 0; eff < MAX_EFFECT_INDEX; ++eff)
        {
            ...
            {
                if (sSpellMgr.IsRankSpellDueToSpell((*i)->GetSpellProto(), spellInfo->Id) && CompareAuraRanks((*i)->GetSpellProto()->Id,spellInfo->Id) > 0)
                    return true;
            }
        }
    //}
